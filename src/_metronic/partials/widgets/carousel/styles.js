 ...
 * @returns {String}
 */
export function getBoxShadow(position, alignment, disable_box_shadow, current_index, index) {
  if (!disable_box_shadow && position === POSITION.CURRENT) {
    if (alignment === ALIGNMENT.HORIZONTAL) {
      if (current_index === index) {
        return '0px 0px 20px white,0px 0px 20px white,0px 0px 20px white,0px 0px 20px white';
      } else {
...
function _getTranslationDistances(spread) {
  ...
  else if (spread === SPREAD.CUSTOMER) {
    prev = '-85%';
    next = '-15%';
